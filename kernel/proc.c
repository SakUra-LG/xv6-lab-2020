...
// per-process kernel page table functions
...
void userinit(void)
{
    ...
    // 把 [0, p->sz) 的用户映射复制到本进程的 kpagetable（清掉 PTE_U）
if (pagecopy(p->pagetable, p->kpagetable, 0, p->sz) != 0)
  panic("userinit pagecopy");
    p->sz = PGSIZE;
...
growproc(int n)
{
  ...
  sz = p->sz;
  if(n > 0){
    // 用户虚拟地址不能长到覆盖内核最低虚拟地址（PLIC）
    // 注意四舍五入到页边界再比较更稳妥：
    uint newsz = uvmalloc(p->pagetable, sz, sz + n);
    if(newsz == 0) return -1;
    if (PGROUNDUP(newsz) >= PLIC) {
      // 回滚（可选）：如果你愿意更严谨，可以在这里把刚分配的又缩回去
      return -1;
    }
    // 仅把新增的区间同步到 kpagetable
    if (pagecopy(p->pagetable, p->kpagetable, sz, newsz) != 0) {
      return -1;
    }
    p->sz = newsz;
  } else if(n < 0){
    uint newsz = uvmdealloc(p->pagetable, sz, sz + n);
    // 仅把删掉的那段从 kpagetable 解除映射；不释放物理页（最后参数 0）
    if (PGROUNDUP(newsz) < PGROUNDUP(sz)) {
      uvmunmap(p->kpagetable,
               PGROUNDUP(newsz),
