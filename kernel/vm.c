...
// 和 mappages 类似，但允许覆盖已有映射（不 panic）。
int
...
// 把 oldpage（用户页表）中的 [begin, end) 区间的映射复制到 newpage（专属内核页表），
// 物理页相同、权限去掉 PTE_U，这样内核可以直接访问同一批物理页的“内核映射”。
int
pagecopy(pagetable_t oldpage, pagetable_t newpage, uint64 begin, uint64 end)
{
  ...
  for (i = begin; i < end; i += PGSIZE) {
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & (~PTE_U);  // 关键：清掉 PTE_U
...
err:
  // 注意：这里对 newpage 仅仅做 unmap，不释放物理页（最后参数为 0）。
  uvmunmap(newpage, PGROUNDUP(begin), (i - PGROUNDUP(begin))/PGSIZE, 0);
  return -1;
}
